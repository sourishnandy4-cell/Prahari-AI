...
export default function GetStartedScreen({ onGetStarted }) {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[9990] flex flex-col items-center justify-between bg-zinc-950 text-zinc-100 overflow-hidden select-none p-8"
    ...
    >
      ...
      <header
        className="w-full max-w-6xl mx-auto flex items-center justify-between z-10 transition-opacity duration-700 ease-out"
        style={{
      ...
      <main
        className="relative flex flex-col items-center justify-center text-center max-w-3xl mx-auto z-10 my-auto transition-opacity duration-700 ease-out"
        style={{
          opacity: isBursting ? 0 : 1,
      ...
      >
        <motion.div
          ...
          className="space-y-6"
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-900/90 border border-zinc-800 text-xs text-zinc-300 font-medium shadow-sm">
            <span>Mangalore Refinery & Petrochemicals (MRPL)</span>
          </div>

          <h1 className="text-5xl sm:text-6xl md:text-7xl font-bold tracking-tight text-white uppercase">
            Sovereign Safety <br /> Intelligence
          </h1>

          <p className="text-base sm:text-lg text-zinc-300 max-w-xl mx-auto tracking-wider font-light italic">
          ...
          <div className="pt-4 flex flex-col items-center gap-3">
            <motion.button
              ...
              onClick={handleClick}
              disabled={isBursting}
              className={`group relative flex items-center justify-center gap-2.5 px-8 py-3.5 rounded-xl bg-white hover:bg-zinc-200 text-zinc-950 font-semibold text-sm shadow-xl transition-all duration-200 cursor-pointer ${
